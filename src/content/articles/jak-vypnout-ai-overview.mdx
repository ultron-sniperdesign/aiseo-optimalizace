...
answer: "Google AI Overviews nelze garantovaně globálně vypnout — Google nenabízí hlavní vypínač. Lze je ale omezit nebo obejít: (1) přes Search Labs experiment v Google účtu (dostupnost závisí na účtu, regionu, rollout fázi); (2) přes web filter `&udm=14`, který aktuálně potlačuje AIO i další SERP prvky; (3) přepnutím výchozího vyhledávače na Brave Search nebo DuckDuckGo. Pro vlastní web pak `data-nosnippet` nebo `nosnippet` direktivy selektivně, podle typu dotazu."
