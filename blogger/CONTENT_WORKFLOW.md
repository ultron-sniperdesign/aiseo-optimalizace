...
- **B1 — Výběr tématu:** vezmi **první volný řádek** (`Publikováno = ne`) — **pokud neplatí přednost níže**.
  ...
  > ⛔ **Přednost k 26. 8. 2026 (zadání admina, `blogger/PRIORITA_2026-08-26.md`):** nejdřív se píší v tomto pořadí řádky **`rezim ai google` (A) → `konec chatgpt atlas` (B) → `nakupovani pres ai` (C)**. Mají to napsané i ve sloupci Pokyny. Teprve po jejich vydání se pokračuje běžným pořadím fronty (a v něm dál platí starší přednost otevřených témat k AI Mode). Podklady k prioritě: `_source/_keyword-research/rezim-ai-cluster-20260826/insights.md` a `_source/_keyword-research/trend-ai-vyhledavani-20260825/insights.md` — číst před psaním. **Nejdřív přečti sloupec C (Pokyny)** — některé řádky mají varování (kanibalizace, „updatovat existující místo nového", riziko překryvu s pilířem). Pokud pokyn říká *nepsat / updatovat existující*, vyřeš řádek podle něj (např. UPDATE `geo-optimalizace.mdx` + bump `updated:`), ne slepě nový článek. Když řádek vyústí v „nepsat", nastav `E` smysluplně a přejdi na další volný.
