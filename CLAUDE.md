...
anglicismy vozí dál, protože audit obsahu je hledal jen v blogu:

```bash
python3 blogger/jazyk-check.py <soubor.mdx> --slovnik blogger/JAZYK_SLOVNIK.md
```

Nález je buď oprava textu, nebo — když jde o vlastní jméno, oficiální název zásady
nebo rozepsanou zkratku — **doložená výjimka ve slovníku** (lookahead/lookbehind v regexu
plus vysvětlení ve sloupci „Odkud"). Cíl je 0 nálezů; „zbytek jsou známé výjimky" neplatí,
protože ta znalost se do dalšího běhu nepřenese. Nové pravidlo = bump verze v hlavičce slovníku.
...
- **Komponentě nejde předat CSS proměnná přes `style="…"`** — `Answer.astro` (a spol.)
  atribut `style` nepropouští na kořenový prvek, takže `<Answer style="--answer-pb: 0">`
...
- **Placený audit se jmenuje „Audit AI viditelnosti“, ne „AI SEO audit“** (rozhodnutí
  ...
  v AI nástrojích**: co o značce říkají ChatGPT, Perplexity, Přehled od AI a režim AI,
  jestli k webu mají roboti přístup, jestli je z čeho citovat a kde jsou obsahové mezery.
  Slovo „SEO audit“ smí na webu stát **jen jako kontrast** („není to klasický SEO audit“).
  Platí pro titulky, H1, krátké odpovědi, FAQ, JSON-LD (`serviceType`), llms.txt, CTA
  i navigaci. **Past při přejmenování:** grep na „AI SEO audit“ minul variantu
  **„SEO a AI audit“**, která byla v pěti sekcích — hledej i ji. Blog nese starý název
...
- **FAQ jen přes komponentu `components/blocks/Faq.astro` — nikdy ručně** (od 2026-09-13).
  ...
  styly číslování, tři formáty odpovědí, čtyři způsoby strukturovaných dat. `/audit/` držel
  FAQ dvakrát (HTML pro stránku, prostý text pro schéma) a **6 z 10 odpovědí se rozešlo** —
  Google dostával jinou odpověď než čtenář. Komponenta vynucuje: **jeden vzhled** (akordeon,
  první otázka otevřená, bez čísel), **jeden zdroj** (viditelný text i FAQPage JSON-LD z týchž
  `items` — stránka FAQ do `jsonLd` v BaseLayout NEpředává), **jeden formát** (mini markdown,
  typ `FaqItem` v `_types.ts`). Rám sekce nasazuje sama; obal, který odsazení už dává, nastaví
  `--faq-px: 0` (a `--faq-my: 0`, když řeší i svislý rytmus) — jinak FAQ začne o 58 px vpravo,
  stejně jako se to stalo u FreeStripu. Akcent následuje `--d` stránky. **Nové FAQ = `<Faq items={…} />`,
  žádná vlastní třída, žádné vlastní schéma.** Změna designu FAQ = změna v jednom souboru.
- **Krátká odpověď jen přes komponentu `components/blocks/Answer.astro`** (od 2026-09-14).
  Do té doby byla ve **12 souborech** na 238 stránkách: čtyři pravé okraje karty, pět velikostí
  štítku a pět popisků („Série“, „Co je AI viditelnost“…); šablona série v komentáři přiznávala
  „kopie z článku“. Komponenta vynucuje **vzhled ze sekcí** (karta na `--w-wide`, štítek z tokenů
  `--label-*`), **dva štítky podle významu** — `kind="definice"` (co je X: sekce, slovník, detail
  služby) a `kind="odpoved"` (článek odpovídá na otázku) — a **mini markdown** v textu přes
  `renderInlineMarkdown` (dřív `renderFaqMarkdown`). Velikost textu a odsazení jdou z šířky bloku
  (`cqi`), takže tentýž blok sedí v pásu přes rám i ve sloupci hero. Rám nasazuje sama; obal,
  který odsazuje, nastaví `--answer-px: 0`, svislé odstupy `--answer-mt` / `--answer-pb`. Pozor
  na sčítání: kde se dřív spodní okraj karty **slil** s horním okrajem další sekce (detail služby),
  patří `--answer-pb: 0`, jinak mezera vyroste o 40 px. Doplňkový obsah (rozlišení pojmů,
  datum hesla) jde do slotu.
- **H1 sekcí se skládá ze dvou zdrojů — `title` s pomlčkou H1 rozbije** — `src/pages/[slug].astro:67`
  bere `entry.data.title.split(" — ")` a do H1 vykreslí `abbr` (velký badge) + **jen část za pomlčkou**.
  Titulek „Kterou disciplínu řešit první — matice podle typu firmy“ proto vyrobil H1
  **„MATICE matice podle typu firmy“** (naměřeno 2026-09-08 na `/rozhodovaci-matice/`). Ze zdroje to
  není vidět a build neprotestuje. **Buď psát `title` bez „ — “** (pak se vykreslí celý, jako
  `/prakticky-postup/`), **nebo ve tvaru `ZKRATKA — popis`** jako `/seo/`, `/geo/`, `/aeo/`.
  Po každé změně `title` u sekce zkontrolovat H1 v prohlížeči.
